        ...
        if choice_id == i: 
            ...
            if target == '1.0':
               ...
               # st.markdown(f'<p style="color:#FF0000;font-size:24px;">{res_t}</p>', unsafe_allow_html=True) 
                ...
                st.write('Shap force plot show how features contributed to the model’s prediction for a specific customer')
                st_shap(shap.force_plot(expected_values1[1], shap_values1[1][choice_id - 100002],data_api[data_api.index==i]))
                #st.set_option('deprecation.showPyplotGlobalUse', False) 
                ...
                st.write('Shap force plot show how features contributed to the model’s prediction for a specific customer')
                st_shap(shap.force_plot(expected_values0[0], shap_values0[0][choice_id - 100002],data_api[data_api.index==i]))
